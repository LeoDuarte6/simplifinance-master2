...
class PaymentService {
    ...
    async createSubscriptionWithDate(subscriptionData) {
        try {
            const { planName, planPrice, startDate, existingCustomerProfile } = subscriptionData;

            if (!existingCustomerProfile) {
                throw new Error('Customer profile is required for subscription date update');
            }

            const { customerProfileId, customerPaymentProfileId, customerAddressId } = existingCustomerProfile;

            logger.info(`Creating subscription with custom start date: ${startDate}`);

            // Create subscription from existing profile with custom start date
            const subscriptionResult = await this.createSubscriptionFromProfile({
                customerProfileId,
                customerPaymentProfileId,
                customerAddressId,
                planName,
                planPrice,
                startDate: startDate // Use the provided start date
            });
            ...
            if (subscriptionResult.success) {
                return {
                    success: true,
                    subscriptionId: subscriptionResult.subscriptionId,
                    message: 'Subscription created with custom billing date successfully'
                };
            } else {
                return {
                    success: false,
                    message: subscriptionResult.message || 'Failed to create subscription with custom date'
                };
            }

        } catch (error) {
            logger.error('Error creating subscription with custom date:', error);
            return {
                success: false,
                message: error.message || 'Failed to create subscription with custom date'
            };
        }
    }
}
